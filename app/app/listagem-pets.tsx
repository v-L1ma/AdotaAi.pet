...
export default function ListagemPets(){
    ...
    useEffect(() => {
        filteredPets.forEach((pet) => {
            if (imageRatios[pet.link_foto]) return;

            Image.getSize(
                pet.link_foto,
                (imgWidth, imgHeight) => {
                    const ratio = imgWidth / imgHeight;
                    setImageRatios((prev) => ({ ...prev, [pet.link_foto]: ratio }));
                },
                () => {
                    setImageRatios((prev) => ({ ...prev, [pet.link_foto]: 1 }));
                }
            );
        });
    ...
    const masonryItems = useMemo(
        () => filteredPets.map((pet, index) => {
            const ratio = imageRatios[pet.link_foto] ?? 1;
    ...
    function toggleEspecie(value: especie) {
        ...
        <View style={styles.screenWrapper}>
            <View style={styles.content}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
                    ...
                    <TouchableOpacity
                        style={[styles.chip, especie === "Cão" && styles.chipActive]}
                        onPress={() => toggleEspecie("Cão")}
                    >
                        <Text style={especie === "Cão" ? styles.chipActiveText : styles.chipText}>Cachorros</Text>
                    </TouchableOpacity>
                        ...
                        <View style={styles.container}>
                            ...
                            >
                                ...
                                <View style={styles.optionRow}>
                                    <TouchableOpacity 
                                    style={[styles.button,  especie === "Cão" && styles.selected]}
                                    onPress={() => setEspecie("Cão")}>
                                        <Text style={[styles.buttonText, especie === "Cão" && styles.selectedText]}>Cachorro</Text>
                                    </TouchableOpacity>
